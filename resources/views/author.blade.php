                ...
                @foreach($data as $item)
                    <div class="col-lg-4 col-sm-6 author-block">
                        <div class="author-card text-center">
                            ...
                            @else
                                <img class="author-image" src="{{\Storage::url($item->avatar)}}">
                            @endif
                            <h3 class="mb-2"><a href="{{route('author.detail', $item->id)}}" class="post-title">{{$item->name}}</a></h3>
                            <p class="mb-3">{{$item->job}}</p>

                            <a class="post-count" href="author-single.html#post"><i class="ti-pencil-alt mr-2"></i><span class="text-primary">{{$item->total}}</span> Bài viết của tác giả này</a>
